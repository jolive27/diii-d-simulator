...
- **Expected.** Cell averages of `J0(a1 rho) exp(−a1²)`. Also report (not judged) the point value at `rho = 0.5`, by linear interpolation between the two neighbouring cell centres (DD-16), against `J0(a1/2) exp(−a1²)`.
...
- **Pass/fail.** Pass iff `L2rel ≤ 1e-4` **and** every `u_i` lies within `[min u(0) − 1e-14 M, max u(0) + 1e-14 M]`, `M = max|u_i(0)|` (discrete maximum principle, DD-21: an `M`-matrix scheme must not create new extrema; for this nonnegative mode that is the no-negative-values rule).
...
- **Tolerance.** `L2rel ≤ 1e-3` (amended from `1e-4`; DD-6). Uniform-mode total conserved to `1e-12` relative. Production-reported edge face flux exactly `0`; a test-side reconstructed delivered flux `≤ 1e-12 × max_j |Φ_j| + 100 ε M ΔV_N / dt` (DD-25 roundoff floor; the reconstruction residual grows as `1/dt`). *Scratch re-check (fix-up):* spatial error alone `2.74e-4`, total at `N_rho = 256`, `dt = 1e-6`: `3.82e-4` (exact discrete backward-Euler solution via eigendecomposition), i.e. a margin of about 2.6 to `1e-3`.
...
- **Tolerance (DD-24).** `|Σ x_i ΔV_i − X_0D(t)| / max(X_0D(t), X_0D(0)) ≤ 1e-9` at every step, with the un-floored ratio `|·|/X_0D(t)` reported (validation observed a worst un-floored `1.116e-9` on a corner whose `W_i` had decayed ~100×; about `1e-12…1e-10` on the default shot at `N_rho = 64`).
...
- **Anti-tautology (mutation) procedure.** On a copy of a clean DEFAULT trace (`N_rho = 64`, `dt = 0.002`), apply, one at a time, all at step `750` (`t = 1.5 s`, heating on): (a) `+1e-6 · N` (with `N` the 0-D inventory at that step) added to density cell `32` (`1` mutation); (b) `1e-6 · We` (0-D inventory at that step) moved from `w_e` cell `10` to `w_e` cell `50`, and separately from `w_e` cell `10` to `w_i` cell `10`, totals conserved in each case (`2` mutations); (c) the oracle's shape multiplied by `(1 + 1e-6)` for one shape at a time — NBI, ECH, gas, bremsstrahlung, Ohmic (`5` mutations, a `1e-6` normalisation error each). Rejection bounds (DD-16, DD-20): mutations (a) and (b) must give a maximum normalised residual `>= 1e-8`; each shape mutation (c) must give `>= max(1e-10, 100 × r_clean)`, `r_clean` being the clean trace's maximum residual at step `750`. The ratio residual/bound is recorded for every mutation.
- **Convergence procedure.** None.
- **Pass/fail.** Pass iff the clean traces of all completing PARITY runs are within tolerance **and** all eight mutations (1 + 2 + 5) are rejected (each ratio residual/bound `>= 1`). A mutation that passes means the oracle is tautological and fails the check.
...
## 4. Disposition of earlier deviations (all resolved by DD-6..DD-17)
...
- **Resolved at design-review (DD-12).** Evaluation point of `h(rho)` and `jt(rho)`: cell centres `rho_i = (i − 1/2)/N_rho`, then normalisation on the discrete cell volumes (`d2-profiles.md` §6, §15).
