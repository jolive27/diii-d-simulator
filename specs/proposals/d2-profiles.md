...
- **AC-7 structure (DD-17).** Four sub-checks, each on the DEFAULT profile-on run (`N_rho = 64`, `dt = 0.002`) and on the four AC-5 fixtures: (a) axis flux: the reported axis face flux is `Object.is(flux, 0)` for every species at every step (`V'(0) = 0`, no operation may produce `−0` or roundoff); (b) edge flux: `|delivered − prescribed| <= 1e-12 × max(|prescribed|, flux scale)` per species per step, where the flux scale is `N/tauP` for particles and `We/tauE`, `Wi/tauE` for energy (so a prescribed `0` is compared absolutely); (c) normalisation: `|Σ h_i ΔV_i − 1| <= 1e-13` for every shape, every step it is (re)evaluated; (d) discrete maximum principle (DD-21): on the diffusion-only fixtures (STEADY, both eigenmodes) every cell value at every step lies in `[min_i u_i(0) − 1e-14 M, max_i u_i(0) + 1e-14 M]`, `M = max_i |u_i(0)|`; for a non-negative solution this is the no-negative-values rule, and for the signed reflecting eigenmode `J0(b1 rho)` it is the correct monotone-scheme property (the constraint check is disabled on signed verification fixtures, which the verification-only entry allows). Pass iff all four hold on every listed case; every maximum is reported.
