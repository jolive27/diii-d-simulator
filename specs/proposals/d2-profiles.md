...
- **AC-3 integral parity (DD-24).** Relative parity `|∫ x V' drho − X_0D(t)| / max(X_0D(t), X_0D(0)) <= 1e-9` (Section 9) for `X = N, We, Wi` at every step, the un-floored ratio reported, for `N_rho in {32,64,128}`, `dt in {0.004,0.002,0.001}`, on every completing case of AC-9.
- **AC-4 ledger and anti-tautology.** Independent test-only oracle (not the production rates) closes `n, w_e, w_i` ledgers `<= 1e-10` per step. Perturbing a copy of the trace by (a) `1e-6` of `N` in one post-step density, (b) `1e-6` of `We` moved between cells or between species with the total conserved, (c) a `1e-6` normalisation error in one shape must each be rejected. Rejection bounds (DD-16, DD-20): for the state perturbations (a) and (b) the oracle must report a ledger residual `>= 1e-8` relative; for each shape perturbation (c), applied separately to every shape (NBI, ECH, gas, bremsstrahlung, Ohmic), the residual must be `>= max(1e-10, 100 × r_clean)` where `r_clean` is the clean trace's maximum residual at the same step (the shape terms are physically small per step, so their detectability is judged against the closure noise, not an absolute number). A residual below its bound for any perturbation fails the check.
...
- **AC-7 structure (DD-17).** Four sub-checks, each on the DEFAULT profile-on run (`N_rho = 64`, `dt = 0.002`) and on the four AC-5 fixtures: (a) axis flux: the reported axis face flux is `Object.is(flux, 0)` for every species at every step (`V'(0) = 0`, no operation may produce `−0` or roundoff); (b) edge flux: `|delivered − prescribed| <= 1e-12 × max(|prescribed|, flux scale) + 100 ε M ΔV_N / dt` per species per step (DD-25), where the flux scale is `N/tauP` for particles and `We/tauE`, `Wi/tauE` for energy, `ε = 2^-52`, `M = max_i |x_i|`, and the second term is the roundoff floor of reconstructing a face flux from the discrete balance (it is what a test-side reconstruction of a prescribed zero flux measures at small `dt`; the production face flux for a prescribed `0` is exactly `0` and must be reported as such); (c) normalisation: `|Σ h_i ΔV_i − 1| <= 1e-13` for every shape, every step it is (re)evaluated; (d) discrete maximum principle (DD-21): on the diffusion-only fixtures (STEADY, both eigenmodes) every cell value at every step lies in `[min_i u_i(0) − 1e-14 M, max_i u_i(0) + 1e-14 M]`, `M = max_i |u_i(0)|`; for a non-negative solution this is the no-negative-values rule, and for the signed reflecting eigenmode `J0(b1 rho)` it is the correct monotone-scheme property (the constraint check is disabled on signed verification fixtures, which the verification-only entry allows). Pass iff all four hold on every listed case; every maximum is reported.
