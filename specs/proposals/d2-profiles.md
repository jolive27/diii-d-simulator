...
3. On: the original `Shot` fields are still `Object.is`-identical to the off run (0-D is a passenger) except that `modelVersion` reports `'0.2.0'` (DD-8) and an additional optional `profiles` block is present: grid, per-sample `n, T_e, T_i` (existing export units: 1e19 m^-3, keV), edge values (Section 4 rule) with clip flags, edge fluxes, `C_br`, `C_ohm`, parity and ledger errors. `schemaVersion` remains 1 when profiles are off (DD-2); for profile-on runs it is recommended to remain 1 (additive optional field), Director to confirm.
...
- **AC-5 analytic fixtures (amended, DD-6).** All against analytic *cell averages*, norm `L2rel` as defined in `d2-profiles-verification.md` §1.
  ...
  - D2-VER-EIGEN-DIRICHLET: `J0(2.4048 rho) exp(−2.4048² D t/L²)`; `L2rel <= 1e-4` at `t = 1` (`D/L² = 1`), `N_rho = 256`, `dt <= 2e-6` (backward-Euler amplitude error is about `16.7 dt` here; the point value at `rho = 0.5` is also reported by interpolation).
...
- **Shape evaluation point (flagged, not decided here).** Section 6 does not say whether `h(rho)` and `jt(rho)` are evaluated at the cell centre or as cell averages before normalisation on the discrete cell volumes. Normalisation makes the source totals exact either way; the choice changes profile shapes at `O(h²)` and the value of `C_ohm` at `O(h²)`. Not covered by DD-6..DD-10; Director to decide or delegate. The verification anchors do not depend on it (`d2-profiles-verification.md`, D2-VER-DIAGNOSTICS).
